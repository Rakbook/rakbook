...
<body>
  <div class="container">
    ...
    ?>
        ...
        <p>Użytkownicy oczekujący na zatwierdzenie:</p>
        <?php
        $query="SELECT id, user_name, user_nrwdzienniku, activated FROM users WHERE activated=0";
        $result=easyQuery($query);

      if ($result->num_rows > 0) {
    	echo "<table><tr><th>nick</th><th>numer</th><th>zatwierdzić?</th><th>odrzucić?</th></tr>";
    	while($row = $result->fetch_assoc()) {
        	echo "<tr><td>". $row["user_name"]. "</td><td>" . $row["user_nrwdzienniku"]. "</td><td> <form action=\"\" method=\"post\"><input type=\"hidden\" name=\"idToActivate\" value=\"".$row['id']."\"/><input type=\"submit\" value=\"Aktywuj\"/></form></td><td><form action=\"\" method=\"post\"><input type=\"hidden\" name=\"idToReject\" value=\"".$row['id']."\"/><input type=\"submit\" value=\"Odrzuć\"/></form></td></tr>";
    	}
    	echo "</table>";
	} else {
